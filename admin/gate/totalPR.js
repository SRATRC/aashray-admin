...
async function fetchPRResidents() {
  try {
    const response = await fetch(`${CONFIG.basePath}/gate/totalPR?page=${currentPage}&page_size=${pageSize}&search=${encodeURIComponent(searchQuery)}&sort_by=${encodeURIComponent(sortBy)}&sort_order=${encodeURIComponent(sortOrder)}&status=${encodeURIComponent(statusFilter)}`, {
      method: 'GET',
      headers: {
...
async function exportToExcel() {
  ...
  try {
    const response = await fetch(`${CONFIG.basePath}/gate/totalPR?search=${encodeURIComponent(searchQuery)}&sort_by=${encodeURIComponent(sortBy)}&sort_order=${encodeURIComponent(sortOrder)}&status=${encodeURIComponent(statusFilter)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${sessionStorage.getItem('token')}`
    ...
    if (response.ok && result.data && result.data.records) {
      ...
      XLSX.writeFile(wb, `PR_Residents_Report_${statusLabel}_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } else {
