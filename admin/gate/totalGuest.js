...
async function fetchGuestResidents() {
  try {
    const response = await fetch(`${CONFIG.basePath}/gate/totalGuest?page=${currentPage}&page_size=${pageSize}&search=${encodeURIComponent(searchQuery)}&sort_by=${encodeURIComponent(sortBy)}&sort_order=${encodeURIComponent(sortOrder)}`, {
      method: 'GET',
      headers: {
...
async function exportToExcel() {
  ...
  try {
    const response = await fetch(`${CONFIG.basePath}/gate/totalGuest?search=${encodeURIComponent(searchQuery)}&sort_by=${encodeURIComponent(sortBy)}&sort_order=${encodeURIComponent(sortOrder)}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${sessionStorage.getItem('token')}`
    ...
    if (response.ok && result.data && result.data.records) {
      ...
      XLSX.writeFile(wb, `Guests_Report_${new Date().toISOString().slice(0, 10)}.xlsx`);
    } else {
